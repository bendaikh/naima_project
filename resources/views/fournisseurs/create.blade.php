...
@section('content')
    <div class="max-w-2xl space-y-6">
        <h1 class="text-2xl font-semibold text-[#1F2937]">Ajouter un fournisseur</h1>
        <form method="post" action="{{ route('fournisseurs.store') }}" class="space-y-4 rounded-xl border border-[#E5E7EB] bg-white p-6 shadow-sm">
            @csrf
            <div>
                <label for="nom" class="block text-sm font-medium text-[#374151]">Nom *</label>
                <input type="text" name="nom" id="nom" value="{{ old('nom') }}" required class="mt-1 w-full rounded-lg border border-[#E5E7EB] px-4 py-2 focus:border-[#1860E1] focus:ring-1 focus:ring-[#1860E1]">
